...
data_gir34_gir12 <- read_excel("I:/_restricted area/RM/I. Risk Management/1.7 - Modèle interne/Cluster 8/PREDICA/Vérification du provisionnement/Verification de la PRC via calcul des lois/Data/Donnée pour les lois/Data loi d'incidence/data_gir34_gir12.xlsx")
t=data_gir34_gir12
...
plot(xPlage,QX[xPlage+1],xlab="Age",ylab="Qx",type="l",xlim = c(68,88))
...
res <- data.frame(0:120,rowSums(decesVentilesH),rowSums(expoVentilesH),Q_H_Hoem,rowSums(decesVentilesF),rowSums(expoVentilesF),Q_F_Hoem,rowSums(decesVentiles),rowSums(expoVentiles),QX_Hoem)

names(res) <- c("Âge","Décès Hommes","Expo Hommes","QH Hoem","Décès Femmes","Expo Femmes","QF Hoem","Décès Mixte","Expo mixte","Qx Mixtes")



# Taux constant de passage
...
sum(rowSums(decesVentilesH))/sum(rowSums(expoVentilesH))
sum(rowSums(decesVentilesF))/sum(rowSums(expoVentilesF))
sum(rowSums(decesVentiles))/sum(rowSums(expoVentiles))
...
gir <- createWorkbook()
addWorksheet(gir ,"sheet1",tabColour = 'blue')

writeData(gir,sheet='sheet1',x=res,sep=',')
...
# saveWorkbook(gir, file = "I:/_restricted area/RM/I. Risk Management/1.7 - Modèle interne/Cluster 8/PREDICA/Vérification du provisionnement/GIR34 vers GIR12.xlsx", overwrite = TRUE)
...
obsTable <- mortalityTable.period(name = "IX GIR34 vers GIR12 Hommes" , ages = 0:120, deathProbs =QX_Hoem ,exposures = rowSums(expoVentiles))
...
obsTable.smooth1 = whittaker.mortalityTable(obsTable, lambda = 5, d = 2, name.postfix = " smoothed (d=2, lambda=1)")
...
plot(0:120,deathProbabilities(obsTable),type='l')

lines(0:120,deathProbabilities(obsTable.smooth),type='l',col='red')
lines(0:120,deathProbabilities(obsTable.smooth1),type='l',col='blue')
lines(0:120,deathProbabilities(obsTable.smooth2),type='l',col='green')

res <- data.frame(0:120,rowSums(decesVentiles),rowSums(expoVentiles),QX_Hoem,deathProbabilities(obsTable.smooth1))
names(res) <- c('Âge','Deces','Expo','Taux bruts','W_H')

write.csv(res,file = "I:/_restricted area/RM/I. Risk Management/1.7 - Modèle interne/Cluster 8/PREDICA/Vérification du provisionnement/WH.csv")
